...
function applyOperation (type, bits, otherBits) {
	...
	for (var i = 0; i < bits.length; i++) {
		switch (type) {
			case 'not':
				applyOperationResult[i] = bits[i] === 0 ? 1 : 0;
			break;
			case 'or':
				applyOperationResult[i] = bits[i] === 0 && bits[i] === otherBits[i] ? 0 : 1;
			break;
			case 'nor':
				applyOperationResult[i] = bits[i] === 0 && bits[i] === otherBits[i] ? 1 : 0;
			break;
			case 'xor':
				applyOperationResult[i] = bits[i] === otherBits[i] ? 0 : 1;
			break;
			case 'xnor':
				applyOperationResult[i] = bits[i] === otherBits[i] ? 1 : 0;
			break;
			case 'and':
				applyOperationResult[i] = bits[i] * otherBits[i];
			break;
			case 'nand':
				applyOperationResult[i] = bits[i] === 1 && bits[i] === otherBits[i] ? 0 : 1;
			break;
		}
	}
